...
func TestAlphabetFix58Length(t *testing.T) {
	crashed := false
	alphabetStr := "o23456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"

	defer func() {
		if err := recover(); err != nil { //如果代码不报错则输出提示语句
			t.Errorf("您传入的参数不符合规则，请重新定义！")
			crashed = true
		}
		if !crashed {
...
func TestUnicodeAlphabet(t *testing.T) {
	...
	for _, testItem := range testCases {
		result := Encode([]byte("恶意代码"), myAlphabet)
		fmt.Println("加密结果为：", string(result))
...
/**
对加密做TPS测试		 1000000	      1097 ns/op
*/
func BenchmarkEncode(b *testing.B) {
	for i := 0; i < b.N; i++ {
		i2 := []byte(testCases[i%len(testCases)][0])
		Encode(i2, BitcoinAlphabet)
	}
}

// 对解密做TPS测试		2000000	       742 ns/op
