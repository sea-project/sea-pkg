...
func div(a, b uint8) uint8 {
	...
	log_a := logTable[a]
	log_b := logTable[b]
	diff := (int(log_a) - int(log_b)) % MaxShares
	if diff < 0 {
		diff += MaxShares
	}
...
func mult(a, b uint8) (out uint8) {
	...
	log_a := logTable[a]
	log_b := logTable[b]
	sum := (int(log_a) + int(log_b)) % MaxShares
...
func Split(secret []byte, parts, threshold int) ([][]byte, error) {
	...
	if m > MaxSecretBytes {
		return nil, fmt.Errorf("invalid secret length: %d(gt %d bytes)", m, MaxSecretBytes)
	}
	if threshold < 2 {
	...
	xCoordinates := mathrand.Perm(MaxShares)
	...
	out := make([][]byte, parts)
	for idx := range out {
		out[idx] = make([]byte, m+1)
		out[idx][m] = uint8(xCoordinates[idx]) + 1
	}
